...
export async function symEncrypt(key: webcrypto.CryptoKey, data: string): Promise<string> {
  ...
  const iv = webcrypto.getRandomValues(new Uint8Array(16)); // Generates a random IV
  const encrypted = await webcrypto.subtle.encrypt(
    {
      name: "AES-CBC",
      iv: iv,
...
export async function symDecrypt(strKey: string, encryptedDataWithIv: string): Promise<string> {
  ...
  // Extracts IV which was prepended to the encrypted data
  const iv = new Uint8Array(encryptedDataWithIvBuffer, 0, 16);
  const encryptedData = new Uint8Array(encryptedDataWithIvBuffer, 16);
